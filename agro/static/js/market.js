...
(function () {
  ...
  function showError(msg) {
    ...
    const banner = document.getElementById('marketSetupBanner');
    if (banner && (msg.includes('AGMARKNET') || msg.includes('503'))) {
  ...
  async function fetchPrices(page = 1, append = false) {
    ...
    try {
      ...
      showError(
        msg.includes('AGMARKNET') || msg.includes('503')
          ? msg + ' — Get a free key at https://data.gov.in/ then run: python manage.py sync_mandi_prices'
          : msg
      );
    } finally {
      showLoading(false);
  ...
  function renderResults(data, append) {
    ...
    if (els.source) {
      ...
      let apiNote = '';
      const isLive =
        src.includes('data.gov') ||
        src.includes('Agmarknet') ||
        src.includes('cached_live') ||
        src.includes('cache');
      if (isLive) {
        apiNote = ' <span class="text-success"><i class="fas fa-circle text-success" style="font-size:0.5rem"></i> Live government mandi data</span>';
      } else if (!data.api_configured) {
        apiNote =
          ' <span class="text-warning">Reference data — add free AGMARKNET_API_KEY from data.gov.in, then run: python manage.py sync_mandi_prices</span>';
      } else {
        apiNote = ' <span class="text-warning">Using fallback data — run sync_mandi_prices or adjust filters</span>';
      }
