...
(function () {
  ...
  fetch('/api/market-prices/?' + marketQs, { credentials: 'same-origin' })
    ...
    .then(({ ok, data }) => {
      ...
      const isLive =
        src.includes('data.gov') || src.includes('Agmarknet') || src.includes('cached');
      const srcNote = isLive
        ? ' <span class="text-success">(live mandi data)</span>'
        : ' <span class="text-warning">(reference — add AGMARKNET_API_KEY + sync_mandi_prices)</span>';
